...
- (id)initWithPoint:(CGPoint)point withGrid:(SquareGrid *)grid {
    if (self = [super initWithPoint:point withGrid:grid]) {
        self.squareScalingDuration = SQUARE_DEMO_DURATION;
        [self beginAnimation];
    }
    return self;
}
...
- (void)beginAnimation {
    [UIView animateWithDuration:self.squareScalingDuration
                     ...
                     animations: ^{
                         CATransform3D layerTransform = CATransform3DMakeScale(SQUARE_DEMO_ENDING_SIZE / SQUARE_DEMO_STARTING_SIZE, SQUARE_DEMO_ENDING_SIZE / SQUARE_DEMO_STARTING_SIZE, 1.0);
